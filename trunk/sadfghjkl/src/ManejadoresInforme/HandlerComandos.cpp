...
void HandlerComandos::insertar_en_hash_titulo(int offset) {
	...
	if (reg.autor != "-1"){
		...
		int clave = this->funcion_hash_titulo(reg.titulo);
...
void HandlerComandos::eliminar_de_hash_palabra(int offset) {
	...
	if (reg.autor != "-1"){
		...
		int clave;
		list < string > palabras = reg.palabras;
		list < string > ::iterator it;
		for (it = palabras.begin(); it != palabras.end(); ++ it) {
...
void HandlerComandos::insertarEnArbol (int tipoArbol, int offset){
	...
	if (puntero){
		...
		if (tipoArbol == 1){
			stringstream ss;
			ss << offset;
			Elementos* elemento = new Elementos(new Clave(reg.autor), new CadenaBytes(reg.autor), new CadenaBytes(ss.str()));
			...
			stringstream ss;
			ss << offset;
			Elementos* elemento2 = new Elementos(new Clave(reg.editorial), new CadenaBytes(reg.editorial), new CadenaBytes(ss.str()));
...
bool HandlerComandos::eliminarEnArbol(int tipoArbol, int offset) {
	...
	if (reg.autor != "-1"){
		if (tipoArbol == 1){
			...
			stringstream ss;
			ss << offset;
			Elementos* elemento = new Elementos(new Clave(reg.editorial), new CadenaBytes(reg.editorial), new CadenaBytes(ss.str()));
			retorno = arbol->borrar(elemento);
			delete elemento;
			delete arbol;
		}
