...
int PersistorHash::guardar_bloque(const CadenaBytes& cadena) {
	return this->handler_bloques.guardar_bloque((char*)cadena.toString().c_str());
}

void PersistorHash::guardar_bloque(const CadenaBytes& cadena, int num_bloque) {
	this->handler_bloques.guardar_bloque((char*)cadena.toString().c_str(), num_bloque);
}

void PersistorHash::recuperar_bloque(int num_bloque, CadenaBytes& cadena) {
